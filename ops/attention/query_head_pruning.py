...
from .triton_kernel.prefill import HeadSparsePrefill
from .triton_kernel.decode import HeadSparseDecode
...
class HeadSparseAttentionKernel(_PruningAttentionKernel):
    """
    Kernel Implementation:

    Token-wise head sprase attention for prefill & decode

    For a head active mask [seqlen, head_num] in:\\
    [1, 0, 1, 1]\\
    [0, 1, 1, 0]\\
    [0, 0, 0, 0]\\
    [1, 1, 1, 1]\\
    with batch_size = 1, seqlen = 4, head_num = 4,
    we can regroup them into head-by-head manner, like:
    ...
    then chunking them head-by-head, and finally get the tiling across batch, head, and seqlen
    """
    ...
    def base_prefill(
        ...
        impl: Optional[str]='dense',
        **kwargs,
    ):
        """
        An implementation of token-wise head sparse attention (causal) in prefill.
        ...
        Args:
            ...
            route_mask: torch.Tensor with shape [batch_size, query_length, num_query_heads], the mask for active heads, which including other mask for query (e.g., left padding mask)
            pad_offset: torch.Tensor with shape [batch_size,], the left padding offset for key and value
        Returns:
            out: torch.Tensor with shape same as q
        """
        return HeadSparsePrefill.kernel(
            q=q,
            k=k,
            v=v,
    ...
    def base_decode(
        ...
        **kwargs,
        ...
        """
        An implementation of token-wise head sparse attention (causal) in decode with gemv.

        The main pipeline is from FLA's flash-attn kernel.

        Args:
            q: torch.Tensor with shape [batch_size, 1, num_heads_k, head_dim]
            ...
            route_mask: torch.Tensor with shape [batch_size, seqlen_q, num_heads_q], the mask for active heads, which including other mask for query
            pad_offset: torch.Tensor with shape [batch_size,], the left padding offset for key and value
        Returns:
            out: torch.Tensor with shape [batch_size, 1, num_heads_k, head_dim]
        """
        return HeadSparseDecode.kernel(
            q=q,
            k=k,
            v=v,
            route_mask=route_mask,
    ...
    def _ref_forward(
        ...
        if route_mask is not None: o.masked_fill_(route_mask.logical_not()[:, :, :, None], 0)
        return o
    ...
    def precision_diff(self, repeat: Optional[int]=3, **kwargs):
        ...
        for is_prefill in [True, False]:
            for i in range(repeat):
                ...
                route_mask = torch.rand([bsz, seqlen_q, num_q_heads], device=device) > sparsity
                attention_mask = torch.ones((bsz, seqlen_kv), dtype=torch.bool, device=device)
                for j in range(bsz): attention_mask[j, :j] = False
                
                if is_prefill: route_mask = route_mask & attention_mask[:, :, None]
                ...
                out = self.forward(deepcopy(q), deepcopy(k), deepcopy(v), route_mask, pad_offset, estimated_sparsity=sparsity, prefill_impl='sort')
    ...
    def get_benchmark(self, **kwargs):
        ...
        if bench_name == 'seqlen':
            plot_name = f'head-sparse-attn-bsz{bsz}-sparsity{int(sparsity * 100)}-{mode}'
            active_mask = torch.tensor([False, True], device='cpu').repeat(bsz, bench_range[-1], num_q_heads)
        elif bench_name == 'bsz':
            plot_name = f'head-sparse-attn-seqlen{seqlen}-sparsity{int(sparsity * 100)}-{mode}'
            active_mask = torch.tensor([False, True], device='cpu').repeat(bench_range[-1], seqlen, num_q_heads)
        elif bench_name == 'sparsity':
            plot_name = f'head-sparse-attn-tokenwise-bsz{bsz}-seqlen{seqlen}-{mode}'
            active_mask = torch.tensor([False, True], device='cpu').repeat(bsz, seqlen, num_q_heads)
        else:
...
def run_test(*args, **kwargs):
    ...
    kernel = HeadSparseAttentionKernel()
...
def run_once(bsz: int, seqlen: int, sparsity: float, num_q_heads: int, num_kv_heads: int, head_dim: int, mode: str, device: str, dtype: torch.dtype, impl: list):
    ...
    route_mask = torch.tensor([False, True], device=device).repeat(bsz, seqlen_q, num_kv_heads)
    pad_offset = torch.zeros((bsz,), dtype=torch.int32, device=device)
    for _impl in impl:
        res = kernel.forward(
            q=q,
