...
# SM120 gather & scatter fuse flash attention, with TMA gather load Q, TMA load K, V
...
def issue_load_KV(
    producer: gl.uint32,
    mK_desc: tma.tensor_descriptor,
    mV_desc: tma.tensor_descriptor,
    ...
    mbarrier.expect(k_bar, mK_desc.block_type.nbytes)
    tma.async_copy_global_to_shared(mK_desc, [mK_base + producer * BLOCK_N, 0], k_bar, sK.index(index), pred=pred)

    mbarrier.expect(v_bar, mV_desc.block_type.nbytes)
    tma.async_copy_global_to_shared(mV_desc, [mK_base + producer * BLOCK_N, 0], v_bar, sV.index(index), pred=pred)
...
def query_sort_prefill_sm120_impl(
    mQ_desc: tma.tensor_descriptor, # TMA in [B * HQ * LQ, D]
    mK_desc: tma.tensor_descriptor,
    mV_desc: tma.tensor_descriptor,
    mO,
    ...
    mQ_base = (tidx * HEAD_NUM_Q + tidy) * SEQ_LEN_Q
    mK_base = (tidx * HEAD_NUM_K + tidy // HEAD_GROUP_NUM) * SEQ_LEN_K
    ...
    sV = gl.allocate_shared_memory(dtype, [BLOCK_N, HEAD_DIM], cfg.sV_layout)

    sQ_bar = gl.allocate_shared_memory(gl.int64, [1], mbarrier.MBarrierLayout())
    sK_bar = gl.allocate_shared_memory(gl.int64, [2, 1], mbarrier.MBarrierLayout())
    sV_bar = gl.allocate_shared_memory(gl.int64, [2, 1], mbarrier.MBarrierLayout())

    mbarrier.init(sQ_bar, count=1)
    for i in gl.static_range(2):
        mbarrier.init(sK_bar.index(i), count=1)
        mbarrier.init(sV_bar.index(i), count=1)
    ...
    qk_scale: gl.constexpr = sm_scale * RCP_LN2
    accum_max = gl.full([BLOCK_M], -float('inf'), dtype=gl.float32, layout=gl.SliceLayout(1, cfg.mma_layout))
    accum_sum = gl.full([BLOCK_M], 1.0, dtype=gl.float32, layout=gl.SliceLayout(1, cfg.mma_layout))
    acc = gl.zeros([BLOCK_M, HEAD_DIM], dtype=gl.float32, layout=cfg.mma_layout)

    gather_layout: gl.constexpr = gl.SliceLayout(1, gl.BlockedLayout([4, 1], [1, 32], [cfg.num_warp, 1], [1, 0]))
    
    # route mask handle
    rMask = gl.load(
        route_mask + tidx * SEQ_LEN_Q + tidz * BLOCK_M + gl.arange(0, BLOCK_M, layout=gl.SliceLayout(1, gather_layout)),
        mask=tidz * BLOCK_M + gl.arange(0, BLOCK_M, layout=gl.SliceLayout(1, gather_layout)) < SEQ_LEN_Q,
        other=0,
    )
    ...
    if is_execute:
        # get gather index
        rIndices = gl.load(
            route_indices + tidx * SEQ_LEN_Q + tidz * BLOCK_M + gl.arange(0, BLOCK_M, layout=gl.SliceLayout(1, gather_layout)),
            mask=rMask > 0,
            other=-1,
        )
        mQ_base += rIndices
        mQ_base = gl.where(rMask > 0, mQ_base, -1)

        rPad = gl.load(pad_offset + tidx)

        # load Q, and KV's 1st pipeline
        mbarrier.expect(sQ_bar, BLOCK_M * mQ_desc.block_type.nbytes)
        tma.async_gather(
            mQ_desc, mQ_base, 0,
            barrier=sQ_bar,
            result=sQ,
        ...
        producer = issue_load_KV(producer, mK_desc, mV_desc, sK, sV, sK_bar, sV_bar, mK_base + rPad, BLOCK_N, 2)

        n_start = rPad
        n_end = min(SEQ_LEN_K, gl.max(rIndices) + 1)
        m_min = gl.min(rIndices) + 1
        # wait until Q arrive
        ...
        for n in range(n_start, n_end, BLOCK_N):
            if n + BLOCK_N < n_end:
                producer = issue_load_KV(producer, mK_desc, mV_desc, sK, sV, sK_bar, sV_bar, mK_base + n, BLOCK_N, 2)
            ...
            rP = gl.where(n + gl.arange(0, BLOCK_N) < SEQ_LEN_K, rP, float("-inf"))
            if n + BLOCK_N > m_min: # causal mask
                col_limit_right = (rIndices + 1 - n)[:, None]
                rP = _apply_causal_mask(rP, col_limit_right)
            ...
            rP = gl.convert_layout(rP_16, cfg.rP_layout)
            consumer, acc = issue_mma_PV(consumer, rP, sV, sV_bar, cfg.rV_layout, acc, 2)
        ...
        # write back
        store_layout = gl.BlockedLayout([1, 8], [4, 8], [cfg.num_warp, 1], [1, 0])
        mQ_base = gl.convert_layout(mQ_base, gl.SliceLayout(1, store_layout))
        acc_r2g = gl.convert_layout(acc, store_layout)
        gl.store(
            mO + (mQ_base * HEAD_DIM)[:, None] + gl.arange(0, HEAD_DIM, layout=gl.SliceLayout(0, store_layout)[None, :]),
            acc_r2g,
            mask=(mQ_base > 0)[:, None],
        )
...
class QuerySparsePrefill:
    ...
    def _sort_kernel(
        ...
        **kwargs
        ...
        mQ_desc = TensorDescriptor(
            base=q,
            shape=[B * HEAD_NUM_Q * SEQ_LEN_Q, HEAD_DIM],
            strides=[HEAD_DIM, 1],
            block_shape=[1, HEAD_DIM],
            layout=gl.NVMMASharedLayout.get_default_for([BLOCK_M, HEAD_DIM], gl_dtype)
        )
        mK_desc = TensorDescriptor(
            base=k,
            shape=[B * HEAD_NUM_K * SEQ_LEN_K, HEAD_DIM],
            strides=[HEAD_DIM, 1],
            block_shape=[BLOCK_N, HEAD_DIM],
            layout=gl.NVMMASharedLayout.get_default_for([BLOCK_N, HEAD_DIM], gl_dtype)
        )
        mV_desc = TensorDescriptor(
            base=v,
            shape=[B * HEAD_NUM_K * SEQ_LEN_K, HEAD_DIM],
            strides=[HEAD_DIM, 1],
            block_shape=[BLOCK_N, HEAD_DIM],
        ...
        kernel[grid](
            mQ_desc, mK_desc, mV_desc, out,
            m_sort, m_sort_indices,
            pad_offset,
            SEQ_LEN_Q, SEQ_LEN_K, HEAD_NUM_Q, HEAD_NUM_K, HEAD_DIM,
