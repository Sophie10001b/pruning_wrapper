...
from .triton_kernel.prefill import GroupSparsePrefill
from .triton_kernel.decode import GroupSparseDecode
...
class GroupSparseAttentionKernel(_PruningAttentionKernel):
    """
    Kernel Implementation:

    Token-wise group sprase attention for prefill & decode

    In prefill stage, we use the following loop:\\
    for i in range(num_query_heads // num_key_heads) --> Outer loop across each head in group\\
    for j in cdiv(key_length, BLOCK_N) --> Inner loop for MMA...
    """
    ...
    def base_prefill(
        ...
        **kwargs,
        ...
        Args:
            ...
            route_mask: torch.Tensor with shape [batch_size, query_length, num_key_heads], the mask for active groups, which including other mask for query (e.g., left padding mask)
        ...
        """
        return GroupSparsePrefill.kernel(
            q=q,
            k=k,
            v=v,
    ...
    def base_decode(
        ...
        **kwargs,
        ...
        """
        An implementation of token-wise group sparse attention (causal) in decode with gemv.
        ...
        Args:
            ...
            k: torch.Tensor with shape [batch_size, key_length, num_key_heads, head_dim]
            v: torch.Tensor with shape [batch_size, key_length, num_key_heads, head_dim]
            route_mask: torch.Tensor with shape [batch_size, 1, num_key_heads], the mask for active groups, which including other mask for query (e.g., left padding mask)
            pad_offset: torch.Tensor with shape [batch_size,], the left padding offset for key and value
        Returns:
            out: torch.Tensor with shape [batch_size, 1, num_heads_k, head_dim]
        """
        return GroupSparseDecode.kernel(
            q=q,
            k=k,
            v=v,
            route_mask=route_mask,
    ...
    def _ref_forward(
        ...
        if route_mask is not None:
            ...
            o = rearrange(o, 'b l hk g d -> b l (hk g) d')
        return o
    ...
    def precision_diff(self, repeat: Optional[int]=3, **kwargs):
        ...
        for is_prefill in [True, False]:
            for i in range(repeat):
                ...
                route_mask = torch.rand([bsz, seqlen_q, num_kv_heads], device=device) > sparsity
    ...
    def get_benchmark(self, **kwargs):
        ...
        if bench_name == 'seqlen':
            plot_name = f'group-sparse-attn-bsz{bsz}-sparsity{int(sparsity * 100)}-{mode}'
            active_mask = torch.tensor([False, True], device='cpu').repeat(bsz, bench_range[-1], num_kv_heads)
        elif bench_name == 'bsz':
            plot_name = f'group-sparse-attn-seqlen{seqlen}-sparsity{int(sparsity * 100)}-{mode}'
            active_mask = torch.tensor([False, True], device='cpu').repeat(bench_range[-1], seqlen, num_kv_heads)
        elif bench_name == 'sparsity':
            plot_name = f'group-sparse-attn-tokenwise-bsz{bsz}-seqlen{seqlen}-{mode}'
            active_mask = torch.tensor([False, True], device='cpu').repeat(bsz, seqlen, num_kv_heads)
        else:
...
def run_test(*args, **kwargs):
    ...
    kernel = GroupSparseAttentionKernel()
