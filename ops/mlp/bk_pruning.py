...
from .base import _PruningMLPKernel, DenseMLPKernel
from .triton_kernel.mlp import BKSparseMLP
...
class BKSparseMLPKernel(_PruningMLPKernel):
    """
    Kernel Implementation:

    Token-wise selection of weight's col group

    For route mask with shape [M, num of groups (NG)], we first transpose it to [NG, M], then feed the sorted version to kernels, now each CTA can handle one BK block with different tokens, and skip current block when sum(mask[bk_id * BK, bm_id * BM:bm_id * BM + BM]) == 0
    """
    ...
    def base_prefill(
        ...
        **kwargs,
        ...
        """
        An implementation of token-wise BK sparse mlp, do not support bias for split-k style gemm
        ...
        **kwargs,
        ...
        if w_gate is None: # single mlp layer
            if route_mask is not None:
                res = rearrange(x, 'b l (h d) -> b l h d', h=route_mask.shape[-1])
    ...
    def precision_diff(self, repeat: Optional[int]=3, **kwargs):
        ...
        for ffn_type in ['mlp']:
            for i in range(repeat):
                ...
                route_mask = torch.rand([1, seqlen, hidden_size // group_size], device=device) > sparsity
    ...
    def get_benchmark(self, **kwargs):
        ...
        if bench_name == 'M':
            plot_name = f'BK-sparse-mlp-N{N}-K{K}-G{G}-sparsity{int(sparsity * 100)}-{mode}'
            active_mask = dict(
                up=torch.tensor([False, True], device='cpu').repeat(1, bench_range[-1], K // G),
            )
        elif bench_name == 'N':
            plot_name = f'BK-sparse-mlp--M{M}-K{K}-G{G}-sparsity{int(sparsity * 100)}-{mode}'
            active_mask = dict(
                up=torch.tensor([False, True], device='cpu').repeat(1, M, K // G),
            )
        elif bench_name == 'K':
            plot_name = f'BK-sparse-mlp--M{M}-N{N}-G{G}-sparsity{int(sparsity * 100)}-{mode}'
            active_mask = dict(
                up=torch.tensor([False, True], device='cpu').repeat(1, M, bench_range[-1] // G),
            )
        elif bench_name == 'G':
            plot_name = f'BK-sparse-mlp--M{M}-N{N}-K{K}-sparsity{int(sparsity * 100)}-{mode}'
            active_mask = dict(
                up=torch.tensor([False, True], device='cpu').repeat(1, M, K // bench_range[0]),
            )
        elif bench_name == 'sparsity':
            plot_name = f'BK-sparse-mlp--M{M}-N{N}-K{K}-G{G}-{mode}'
            active_mask = dict(
                up=torch.rand((1, M, K // G), device='cpu'),
            )
        else:
        ...
        def benchmark(M, N, K, G, sparsity, mode, mask, device, dtype, impl, provider):
            ...
            if provider == 'torch':
                ...
                func = partial(
                    ...
                    b_up=up.bias,
                    activation='silu',
                    route_mask=mask['up'][:, :M, :K // G].to(device) > sparsity,
