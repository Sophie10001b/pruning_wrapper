...
def bm_sort_impl(
    ...
    if skip_flag > 0:
        ...
        if ACTIVATION == 'relu':
            acc_gate = tl.maximum(acc_gate, 0)
...
class BMSparseGLU:
    ...
    def _indexing_kernel(
        ...
        **kwargs
        ...
        # Compute up and gate projections
        tmp = x.flatten(0, 1).index_select(dim=0, index=indices)
        res_up = torch.zeros((x.shape[0] * x.shape[1], wu.shape[0]), dtype=x.dtype, device=x.device)
        res_gate = torch.zeros((x.shape[0] * x.shape[1], wg.shape[0]), dtype=x.dtype, device=x.device)
        
        out_up = torch.nn.functional.linear(tmp, wu, bu)
        out_gate = torch.nn.functional.linear(tmp, wg, bg)
        
        res_up[indices] = out_up
        res_gate[indices] = out_gate
        
        if activation == 'silu':
            res_gate = res_gate * torch.sigmoid(res_gate)
        elif activation == 'relu':
            res_gate = torch.nn.functional.relu(res_gate)
    ...
    def kernel(
        ...
        **kwargs
        ...
        """
        Get kernel for token sparse GLU, where x = [M, K], weight = [N, K]\\
        **sort:**\\
...
class BNSparseGLU:
    ...
    def kernel(
        ...
        **kwargs
        ...
        """
        Get kernel for token-aware BN sparse GLU, where x = [M, K], weight = [N, K]\\
        **sort:**\\
        Return the same shape as input, with additional sort prologue for better skipping
        Args:
            x: torch.Tensor with shape [batch_size, seqlen, hidden_size]
